...
每次 session 結束
       ↓
  寫入每日檔案    (~/.hmc/daily/2026-04-20.md)
       ↓  每 3 天
...
這會自動：
- 建立 `~/.hmc/` 目錄結構
- 將 `SessionEnd` hook 注入 `~/.claude/settings.json`（Claude Code）
...
| 平台 | 自動記錄 | MCP 工具 | 說明 |
|------|---------|---------|------|
| **Claude Code** | ✅ 透過 SessionEnd hook | ✅ | 主要支援平台 |
...
我們決定採用 MCP-first 架構搭配 platform adapter 模式...
